...
class ec_point {
 public:
    ...
    ec_point operator*(const BN_new() &n) const {
        BN_new() zero(0);
        ec_point res(gr);
        throw_c(EC_POINT_mul(gr, res.p(), zero.bn(),
                             pt, n.bn(), _BN_new()_ctx::the_ctx()));
        return res;
    }

    bool operator==(const ec_point &other) const {
        return EC_POINT_cmp(gr, pt, other.pt, _BN_new()_ctx::the_ctx()) == 0;
    }
    ...
    std::string to_string(point_conversion_form_t form =
        ...
        char *s = EC_POINT_point2hex(gr, pt, form, _BN_new()_ctx::the_ctx());
        std::string r(s);
        free(s);
        return r;
    }
